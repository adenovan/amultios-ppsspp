...
int AdhocNetworkManager::Init(){
    ...
    if(AdhocManagerInited) return -1;
    
    NOTICE_LOG(AMULTIOS,"Network Multiplexer Started");
    while(amultiosInited){

    }
    NOTICE_LOG(AMULTIOS,"Network Multiplexer Ended");
    return 0;
}
...
int AdhocNetworkManager::Recv(int id, uint8_t * data ,size_t dataSize, SceUID threadID){

}

int AdhocNetworkManager::Term(){
    if(!AdhocManagerInited) return -1;
    NOTICE_LOG(AMULTIOS,"Network Multiplexer Ended");
...
void amultios_sync()
{
    ...
    j.writeString("SessionNickname", g_Config.sNickName);
    j.writeString("SessionMac", g_Config.sMACAddress);
    ...
    int rc = amultios_publish("TOKEN", (void *)publish.c_str(), (int)publish.size(), 2, 0);
            ...
            if (command.at(0).at(0) == '!')
            {
                ...
                else if (command.at(0) == "!sub")
                {
                    ...
                    if (it != ChannelList.end())
                    {
                        ...
                        {
                            int rc = amultios_subscribe((Acl + "/" + selected + "/#").c_str(), 2);
                            if (rc == MOSQ_ERR_SUCCESS)
                            {
                ...
                else if (command.at(0) == "!unsub")
                {
                    ...
                    {
                        int rc = amultios_unsubscribe((Acl + "/" + selected + "/#").c_str());
                        if (rc == MOSQ_ERR_SUCCESS)
                        {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (subit != SubcriptionList.end() && selected != "SYSTEM")
                            {
                                ...
                                if (selected == "WORLD")
                                {
                                    cmList.Add("make sure to use english in world channel!", "Amultios", "SYSTEM", "TEXT");
                                }
                            }
                            else
                            {
...
{
    auto amultios_mqtt = g_amultios_mqtt;
    if (amultios_mqtt->connected && trusted && Acl != "CHAT")
    {
        if (this->selectedRoom == "PRIVATE")
        {
            std::string pubtopic = Acl + "/" + this->selectedRoom + this->privateMessageRoom + "/" + loginInfo.SceNetAdhocctlNickname;
            int rc = amultios_publish(pubtopic.c_str(), (void *)text.c_str(), (int)text.size(), 2, 0);
            if (rc == MOSQ_ERR_SUCCESS)
            {
                cmList.Add(text, loginInfo.SceNetAdhocctlNickname, "PRIVATE");
            }
        }
        else if (this->selectedRoom == "PARTY")
        {
            ...
            {
                std::string pubtopic = Acl + "/" + room + "/" + loginInfo.SceNetAdhocctlNickname;
        ...
        else
        {
            ...
            if (subit != SubcriptionList.end())
            {
                std::string topic = Acl + "/" + this->selectedRoom + "/" + loginInfo.SceNetAdhocctlNickname;
                amultios_publish(topic.c_str(), (void *)text.c_str(), (int)text.size(), 2, 0);
            }
            else
            {
                cmList.Add("Channel " + this->selectedRoom + " is not in your subscription list !sub channelname to listen the chat as subscriber");
                cmList.Update();
            }
        }
    }
...
void addAmultiosPeer(AmultiosNetAdhocctlConnectPacketS2C *packet)
{
    ...
    {
        // Allocate Structure
        peer = (SceNetAdhocctlPeerInfoAmultios *)malloc(sizeof(SceNetAdhocctlPeerInfoAmultios));
        // Allocated Structure
        if (peer != NULL)
        {
            // Clear Memory
            memset(peer, 0, sizeof(SceNetAdhocctlPeerInfoAmultios));
...
void deleteAmultiosPeer(SceNetEtherAddr *mac)
{
    ...
    SceNetAdhocctlPeerInfoAmultios *prev = NULL;
        ...
        try
        {
            ...
            else
            {
                ...
                else if (topic_explode.at(0) == "AUTH")
                {
                    ...
                    json::JsonReader reader(token.c_str(), token.size());

                    const json::JsonGet root = reader.root();
                    loginInfo.SceNetAdhocctlNickname = root.getString("SceNetAdhocctlNickname", "NOT FOUND");
                    loginInfo.SceNetEtherAddr = root.getString("SceNetEtherAddr", "00:00:00:00:00:00");
                    loginInfo.Validate = root.getString("Validate", "");
                    loginInfo.Roles = root.getInt("Roles", 4);
                    loginInfo.Status = root.getString("Status", "Login Failed");
                    loginInfo.Issuer = root.getString("Issuer", "");

                    stateTransfer = root.getString("Validate", "");

                    std::string message = root.getString("Message", "Failed To Contact Authentication Server");
                    int expired = root.getInt("Expired", OPCODE_AMULTIOS_LOGOUT);

                    if (expired == OPCODE_AMULTIOS_LOGOUT)
                    {
                        trusted = false;
                        cmList.Add(message, "Amultios", "SYSTEM", "HEADER");
                        Acl = "CHAT";
                        loginInfo.logedIn = false;
                    }

                    if (loginInfo.Validate != "")
                    {
                        int rc = amultios_subscribe(loginInfo.Validate.c_str(), 2);
                        if (rc == MOSQ_ERR_SUCCESS)
                        {
                            if (expired == OPCODE_AMULTIOS_LOGIN_PASS)
                            {
                                loginInfo.logedIn = true;
                                cmList.Add("Login Verified Synchronize Network", "Amultios", "SYSTEM", "HEADER");
                                Acl = "CHAT";
                            }
                            amultios_sync();
                        }
                ...
                else if (topic == stateTransfer)
                {
                    ...
                    const json::JsonGet root = reader.root();
                    int state = root.getInt("State", OPCODE_AMULTIOS_LOGOUT);
                    ...
                    if (state == OPCODE_AMULTIOS_LOGIN_PASS)
                    {
                        ...
                        if (counter == 0)
                        {
                            ...
                            cmList.Add("Type !help for list of command");
                        }
                    }
                    else
                    {
                        loginInfo.AdhocSwitch = "OFF (Failed Login)";
                        if (counter == 0)
                        {
                            cmList.Add(message, "Amultios", "SYSTEM", "HEADER");
                        }
...
int __AMULTIOS_INIT()
{
    ...
    if (!amultiosInited)
    {
        cmList.Clear();
        loginInfo.counter = 0;
        loginInfo.SceNetAdhocctlNickname.clear();
        loginInfo.SceNetEtherAddr.clear();
        loginInfo.Validate.clear();
        loginInfo.Issuer.clear();
        loginInfo.Roles = 4;
        loginInfo.AdhocSwitch = "OFF (Failed Login)";
...
int AmultiosNetAdhocctlGetPeerList(u32 sizeAddr, u32 bufAddr)
{
    ...
    //need to check module loaded first
    ...
    if (buf == NULL)
    {
        *buflen = countActiveAmultiosPeer() * sizeof(SceNetAdhocctlPeerInfoEmu);
    }

    if (buf != NULL)
    {
        int request_count = *buflen / sizeof(SceNetAdhocctlPeerInfoEmu);
        int remain_buff = *buflen;
        memset(buf, 0, *buflen);
        *buflen = 0;

        DEBUG_LOG(AMULTIOS, "AmultiosNetGetPeerList reqcount %d , actual buff size %d ,struct size %lu", request_count, remain_buff, sizeof(SceNetAdhocctlPeerInfoEmu));
        // minimum buffer >= 152
        if (request_count > 0)
        {
            SceNetAdhocctlPeerInfoAmultios *peer = amultios_peer;
            //iterate and fill the buff
            int pushed = 0;
            for (; peer != NULL && pushed < request_count; peer = peer->next)
            {
                // Fake Receive Time
                if (peer->last_recv != 0)
                    peer->last_recv = CoreTiming::GetGlobalTimeUsScaled();
                ...
                // Copy Peer Info
                buf[pushed].nickname = peer->nickname;
                buf[pushed].mac_addr = peer->mac_addr;
                buf[pushed].last_recv = peer->last_recv;
                pushed++;
            }
            ...
            //give the actual buff size
            *buflen = pushed * sizeof(SceNetAdhocctlPeerInfoEmu);
        }
    }

    peerlock.unlock();
    DEBUG_LOG(AMULTIOS, "AmultiosNetGetPeerList leaving with size %d", *buflen);
    return 0;
}
...
int AmultiosNetAdhocctlDisconnect()
{
    ...
    { // (threadStatus == ADHOCCTL_STATE_CONNECTED)
        ...
        // Clear Peer List
        freeAllActiveAmultiosPeer(amultios_peer);
        INFO_LOG(AMULTIOS, "Cleared Active Peer List.");
...
int AmultiosNetAdhocPdpCreate(const char *mac, u32 port, int bufferSize, u32 unknown)
{
    ...
    if (netAdhocInited)
    {
        ...
        if (mac != NULL && bufferSize > 0)
        {
            ...
            WARN_LOG(AMULTIOS, "AmultiosPdpCreate Returning Invalid Local Mac Address");
    ...
    if (netAdhocInited)
    {
        ...
        if (id > 0 && id <= 255 && pdp[id - 1] != NULL)
        {
            ...
            if (saddr != NULL && port != NULL && buf != NULL && len != NULL && *len > 0)
            {
                ...
                if (flag)
                {
                    DEBUG_LOG(AMULTIOS, "No Message Found addr ");
                    return ERROR_NET_ADHOC_WOULD_BLOCK;
                }
                ...
                if (!flag)
                {
                    ...
                    DEBUG_LOG(AMULTIOS, "Warning Receive Reached  Timeout mac %s[%hu] timeout [%d] len[%d]", getMacString(saddr).c_str(), *sport, timeout, *len);
                }
                return ERROR_NET_ADHOC_TIMEOUT;
            }
